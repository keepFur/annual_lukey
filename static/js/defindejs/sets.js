...
function toggleMenu(menuId, liId) {
    ...
    $('li.list-item-menu').removeClass('open');
...
function modalshow(modalId) {
    if (modalId === 'editprizeModal') {
        ...
        if (selectPrizes.length > 1) {
            ...
            $('#old_onlyUnlucky').val(selectPrizes[0].onlyUnlucky);
        }
...
function submit(modalId) {
    if (modalId == "importUserInfoModal") {  //导入员工信息
        ...
        var onlyUnlucky = $('#old_onlyUnlucky').val().trim();
        ...
        var onlyUnlucky = $('#onlyUnlucky').val().trim();//规则
        var prizes = new Prize(prizeName, size, total, onlyUnlucky);
        addPrize(prizes);
        $("#prizes_table").bootstrapTable("load", getAllPrize());
        $('#' + modalId).modal('hide');
