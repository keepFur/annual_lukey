...
function deleteRecord(eCode, pName) {
    ...
    if (e != null) {
        ...
        console.info("deleteRecord,employee not found" + e);
    }
...
function getARandomECodeByPrize(pName) {

    // console.log("sta:" + new Date().getTime());
    var l = storage.length;
    //是否只抽没有中奖的
    if (getPrize(pName) == null)
        return null;
...
function test(total) {
    ...
    for (var i = 0; i < total; i++) {
        chance[i] = parseInt((r[i] / (times / total) - 1) * 10000) / 100 + "%";
    }
    console.log(r);
    console.log(chance);
}
